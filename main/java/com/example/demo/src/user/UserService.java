...
public class UserService {
    ...
    // 회원정보 프로필 수정(Patch)
    public void modifyUserProfile(PatchProfileReq patchProfileReq) throws BaseException {
        try {
            int result = userDao.modifyUserProfile(patchProfileReq); // 해당 과정이 무사히 수행되면 True(1), 그렇지 않으면 False(0)입니다.
            if (result == 0) { // result값이 0이면 과정이 실패한 것이므로 에러 메서지를 보냅니다.
