...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (name, email, password, nickname, phoneNum) VALUES (?,?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getName(), postUserReq.getEmail(), postUserReq.getPassword(), postUserReq.getNickname(), postUserReq.getPhoneNum()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    // 회원정보 변경
    public int modifyUserProfile(PatchProfileReq patchProfileReq) {
        String modifyUserProfileQuery = "update User set profileImgUrl = ? where userIdx = ? "; // 해당 userIdx를 만족하는 User를 해당 profileImgUrl으로 변경한다.
        Object[] modifyUserProfileParams = new Object[]{patchProfileReq.getProfileImgUrl(), patchProfileReq.getUserIdx()}; // 주입될 값들(profileImgUrl, userIdx) 순
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select userIdx, password,email,nickname,profileImgUrl from User where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
    ...
    public List<GetUserRes> getUsers(int pageNo, int pageSize) {
        String getUsersQuery = "select * from User u join (select userIdx from User order by userIdx asc limit ? offset ?) as temp on temp.userIdx = u.userIdx where status = 'active'"; //User 테이블에 존재하는 모든 회원들의 정보를 조회하는 쿼리
    ...
    public List<GetUserRes> getUsersByNickname(String nickname) {
        ...
        return this.jdbcTemplate.query(getUsersByNicknameQuery,
                (rs, rowNum) -> new GetUserRes(
                        ...
                        rs.getString("nickname"),
                        rs.getString("Email"),
                        rs.getInt("rating")), // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
